...
class RestorePasswordMixin(models.Model):
    ...
    def send_restore_code(self, email=None, phone=None):
        from garpix_user.exceptions import UserUnregisteredException, WaitException
        ...
        if settings.GARPIX_USER.get('TIME_LAST_REQUEST', None):
            if self.restore_date and self.restore_date + timedelta(
                    minutes=settings.GARPIX_USER.get('TIME_LAST_REQUEST')) >= datetime.now(self.restore_date.tzinfo):
