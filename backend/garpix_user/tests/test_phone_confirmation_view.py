...
class TestPhoneConfirmationView:
    ...
    def test_send_code_for_unauthenticated_user_with_preregistration(self, settings):
    ...
    def test_send_code_for_unauthenticated_user_without_preregistration(self, settings):
    ...
    def test_check_code_for_unauthenticated_user_with_preregistration(self, settings):
        settings.GARPIX_USER['USE_PREREGISTRATION_PHONE_CONFIRMATION'] = True
        request = self.factory.post('/phone-confirmation/check-code/', {'phone_confirmation_code': '123456'}, format='json')
        request.user = AnonymousUser()
        response = self.view(request, 'check_code')
    ...
    def test_check_code_for_unauthenticated_user_without_preregistration(self, settings):
        settings.GARPIX_USER['USE_PREREGISTRATION_PHONE_CONFIRMATION'] = False
        request = self.factory.post('/phone-confirmation/check-code/', {'phone_confirmation_code': '123456'}, format='json')
        request.user = AnonymousUser()
        with pytest.raises(NotAuthenticated):
