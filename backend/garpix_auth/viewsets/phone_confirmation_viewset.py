...
class PhoneConfirmationViewSet(viewsets.ViewSet):
    ...
    def send_code(self, request, *args, **kwargs):
        ...
        if user.is_authenticated:
            ...
            if hasattr(settings, 'GARPIX_USE_PREREGISTRATION_PHONE_CONFIRMATION') \
                    and settings.GARPIX_USE_PREREGISTRATION_PHONE_CONFIRMATION:
                serializer = PhonePreConfirmSendSerializer(data=request.data)
                serializer.is_valid(raise_exception=True)
                result = UserSession().send_confirmation_code(serializer.data['phone'])
            else:
    ...
    def check_code(self, request, *args, **kwargs):
        ...
        if user.is_authenticated:
            ...
            if hasattr(settings, 'GARPIX_USE_PREREGISTRATION_PHONE_CONFIRMATION') \
                ...
                serializer.is_valid(raise_exception=True)
                result = UserSession().check_confirmation_code(serializer.data['phone'],
